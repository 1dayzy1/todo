...
export const renderDate = async(containerTasks, date) =>{
    ...
    task.forEach(el => {
        ...
        <a href='https://t.me/${el.username}?text=Добрый день, напоминаю сегодня в ${el.startTime} пройдет наш урок' target="_blank" class="todo__tasks-taskCard-title">${el.title} </ф>
        <div class="todo__tasks-taskCard-time">${el.startTime}-${el.endTime}</div>
